...
generate_args() {
    ...
    ARGS+=("-f ${DATASET_DIR}/${dataset} -d ${datatype} -s ${similarity} -t ${threshold} -x partition -l ${LABEL_PREFIX}ts -m time-static -i 0.25")
    ARGS+=("-f ${DATASET_DIR}/${dataset} -d ${datatype} -s ${similarity} -t ${threshold} -x partition -l ${LABEL_PREFIX}td -m time-dynamic -i 0.25")
    ARGS+=("-f ${DATASET_DIR}/${dataset} -d ${datatype} -s ${similarity} -t ${threshold} -x partition -l ${LABEL_PREFIX}tdw -m time-dynamic -i 0.25 -w")
}
