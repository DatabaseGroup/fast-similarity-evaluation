...
#ifndef SRC_OBJECT_PTR_HH
#define SRC_OBJECT_PTR_HH
...
struct smart_pointer_ops_consistent
    : std::integral_constant<
        ...
        std::is_pointer<decltype(std::declval<Ptr const&>().get())>::value &&
          ...
          std::is_same<decltype(*std::declval<Ptr const&>().get()), decltype(*std::declval<Ptr const&>())>::value> {};
...
template <typename T> struct hash<util::object_ptr<T>> {
  constexpr size_t operator()(util::object_ptr<T> const &p) const
    noexcept {
    return hash<T *>()(p.get());
  }
};
...
typename std::enable_if<
  ...
  util::object_ptr<To>>::type
static_pointer_cast(util::object_ptr<From> p) {
...
typename std::enable_if<
  ...
  util::object_ptr<To>>::type
dynamic_pointer_cast(util::object_ptr<From> p) {
  return dynamic_cast<To *>(p.get());
}

}

#endif  // SRC_OBJECT_PTR_HH
